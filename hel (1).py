    ...
    def yes():  
        ...
        if n == 1:
            ...
            def menstrual_tracker():
                        window_mt = tk.Toplevel(window2)
                        window_mt.title("Menstrual Cycle Tracker")
                        window_mt.geometry(f"{win_w}x{win_h}")
                        
                        # Use a soft background color or image
                        window_mt.configure(bg="#fff0f5") 

                        tk.Label(window_mt, text="Menstrual Cycle Tracker", font=("Georgia", 30, "bold", "underline"), bg="#fff0f5").pack(pady=20)

                        # Input Fields
                        tk.Label(window_mt, text="Enter date of last period (DD):", font=("Arial", 14), bg="#fff0f5").pack(pady=5)
                        day_var = tk.StringVar()
                        tk.Entry(window_mt, textvariable=day_var, font=("Arial", 14)).pack(pady=5)

                        tk.Label(window_mt, text="Enter average cycle length (e.g., 28):", font=("Arial", 14), bg="#fff0f5").pack(pady=5)
                        cycle_var = tk.StringVar()
                        tk.Entry(window_mt, textvariable=cycle_var, font=("Arial", 14)).pack(pady=5)

                        result_txt = tk.Label(window_mt, text="", font=("Arial", 15, "bold"), bg="#fff0f5", fg="#d81b60")
                        result_txt.pack(pady=20)
                        ...
                        def calculate_cycle():
                            try:
                                ...
                                next_period = (day + cycle) % 30 # Simplified for demo
                                if next_period == 0: next_period = 30
                                
                                ovulation = (next_period - 14)
                                if ovulation <= 0: ovulation += 30

                                result_txt.config(
                                    text=f"Estimated Next Period: Around the {next_period}th of next month\n"
                                        f"Estimated Ovulation Day: Around the {ovulation}th of next month"
                                )
                            except ValueError:
                                messagebox.showerror("Error", "Please enter valid numbers")

                        tk.Button(window_mt, text="Predict Cycle", font=("Arial", 14), bg="#d81b60", fg="white", command=calculate_cycle).pack(pady=10)
                        
                        # Calendar Integration
                        tk.Button(window_mt, text="Open Calendar to Log", font=("Arial", 14), bg="black", fg="white", command=cal.load_Cal).pack(pady=10)
                        
                        tk.Button(window_mt, text="Close", font=("Arial", 14), command=window_mt.destroy).pack(pady=10)
            b5 = tk.Button(frame, text="Menstrual Cycle", font=("Arial",20), command=menstrual_tracker, bg="Grey", width=12, height=1)
