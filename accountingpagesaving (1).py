...
def load_acc_page():
    ...
    savingsframe = tk. LabelFrame(window, text="Savings & Investment Plan",bg="#1f2933”,padx=10, pady=10)

    savingsframe. pack(fill="x", padx=20, pady=10)
    monthlyexp = tk. DoubleVar()
    investment per = tk.DoubleVar()
    savings per = tk. DoubleVar()
    tk.Label(savingsframe, text="Monthly Expenditure",bg="#1f2933").grid(row=0, column=0, sticky="w", pady=5)
    tk.Entry(savingsframe, textvariable=monthlyexp).grid(row=0, column=1)
    tk.Label(savingsframe, text="Investment Plan %",bg="#1f2933").grid(row=1, column=0, sticky="w", pady=5)
    tk.Entry(savingsframe, textvariable=investmentper).grid(row=1, column=1)
    tk.Label(savings frame, text="Monthly Savings (%)",bg="#1f2933").grid(row=2, column=0, sticky="w", pady=5)
    tk.Entry(savings frame, textvariable=savingsper).grid(row=2, column=1)
    def calculatesavings():
        exp = monthlyexp. get()
        investamt = (investmentper. get() / 100) * exp
        save_amt = (savingsper.get() / 100) * exp
        messagebox.showinfo("Savings Summary",
                            f"Monthly Expenditure : ₹{exp}\n"
                            f"Investment Amount  : ₹{invest_amt}\n"
                            f"Savings Amount     : ₹{save_amt}"
                            )
    tk.Button(savingsframe, text="Calculate Savings",command=calculatesavings,  bg="#4b5563", fg="white"). grid(row=3, column=0, columnspan=2, pady=10)
window.mainloop()
