...
def load_acc_page():
    ...
    tk.Label(
        window,
        text="Track and analyze your monthly expenses",
        font=("Segoe UI", 11),
        fg="#9ca3af",
        bg="#111827"
    ).pack(pady=(0, 30))
    ...
    savingsframe = tk. LabelFrame(window, text="Savings & Investment Plan",bg="#1f2933",padx=20, pady=20)
    ...
    investmentper = tk.DoubleVar()
    savingsper = tk. DoubleVar()
    tk.Label(savingsframe, text="Investment Plan %",bg="#1f2933").grid(row=1, column=0, sticky="w", pady=10)
    tk.Entry(savingsframe, textvariable=investmentper).grid(row=1, column=1)
    tk.Label(savingsframe, text="Monthly Savings (%)",bg="#1f2933").grid(row=2, column=0, sticky="w", pady=10)
    tk.Entry(savingsframe, textvariable=savingsper).grid(row=2, column=1)
    def calculatesavings():
        exp = total(generalvar)+total(medicalvar)
